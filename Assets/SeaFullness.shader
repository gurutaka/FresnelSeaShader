...
Shader "FresnelReflection"
{
    Properties
    {
        ...
        _DiffusePower ("DiffusePower", Range(0 ,3)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
           Tags {"LightMode"="ForwardBase"}//境光、メインのディレクショナルライト、頂点/SH ライト
           CGPROGRAM
           ...
           #include "UnityCG.cginc"
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                half2 uv = i.uv;
                fixed4 col;
                col.rgb =  tex2D(_EmissionMap, i.uv) * _EmissionColor;//エミッション固定
                
                uv.y += t * _FlowSpeedNormal1;
                //uv.x += t * _FlowSpeedNormal1;
                
                float3 normal1 = UnpackNormal(tex2D(_NormalTex1, uv * _WaveTiling.x));
                
                i.uv.y += t * _FlowSpeedNormal2;
                //i.uv.x += t * _FlowSpeedNormal2;
                float3 normal2 = UnpackNormal(tex2D(_NormalTex2, i.uv * _WaveTiling.y));
                ...
                //ライティング
                float4 diff = saturate(dot(normal, i.lightDir)) * _LightColor0 * _DiffusePower;
